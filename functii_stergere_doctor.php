...
function obtine_id_doctor($conexiune_bd) {
    ...
    // Folosim email și telefon din sesiunea curentă
    ...
    if ($email || $telefon) {
        ...
        $query .= implode(" AND ", $conditii); // Adăugăm condițiile în query
    ...
    return $ID_Doctor; // Returnăm ID-ul sau null dacă nu a fost găsit
}
...
function sterge_studiu_selectat($conexiune_bd, $ID_Studiu) {
    ...
    if ($ID_Doctor) {
        // Ștergem înregistrarea doar dacă ID_Studiu și ID_Doctor corespund
...
function sterge_doctor_din_doctor($conexiune_bd) {
    ...
    if ($ID_Doctor) {
        // Ștergere din tabela consultatie
